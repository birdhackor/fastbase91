...
**先講結論：** 如果你為了真正的平行處理才換到自由執行緒 Python，只要匯入一個沒準備好的套件，這份平行能力就會被默默拿走——而且你不會看到任何錯誤，只會發現程式變慢。fastbase91 特別設計成：匯入它絕不會這樣坑你。

## 它幫你避開的陷阱

自由執行緒 CPython 是 Python 的「no-GIL」版本（3.13 以後，目前仍標為實驗性）。一般 CPython 有一把全域直譯器鎖（GIL），同一時間只允許一條執行緒跑 Python；no-GIL 版本把它拿掉，讓多條執行緒真的能同時跑。

問題在這裡。編譯型擴充（帶有 C 或 Rust 部分的套件，例如 NumPy，或這個套件）必須「主動宣告」自己可以在沒有 GIL 的情況下執行。如果你匯入一個**沒有**宣告支援的擴充，CPython 為了安全，會把 **GIL 重新打開，而且是整個行程都打開**——不只是那個套件。所以只要有一個還沒跟上的相依套件，就會默默把你整支程式打回「一次只能跑一條執行緒」。

最麻煩的是它不會警告你。執行緒照跑，結果也照樣正確——只是又變回一條一條輪流跑，你當初想要的平行能力就沒了。

## 為什麼匯入 fastbase91 是安全的

Python binding 標記了 `#[pymodule(gil_used = false)]`——這是 Rust 原始碼裡的一行，告訴 CPython「這個模組在沒有 GIL 時是安全的」。因為有這個標記，匯入 fastbase91 **不會**觸發上面那個退回動作。你的自由執行緒程式，會繼續維持自由執行緒。

講具體一點：你在自由執行緒 Python 上做了一個服務，讓八條工作執行緒平行編碼各自的資料，並用 fastbase91 來做編碼。匯入它之後，`sys._is_gil_enabled()` 維持 `False`，你的八條執行緒繼續同時跑。要是你當初挑的是一個沒宣告支援的擴充，光是那一次匯入就會把 GIL 打開，八條執行緒就得排隊輪流跑——輸出一樣，吞吐量只剩一小部分。
...
除了常規的 abi3 wheel（涵蓋 CPython 3.11 以上），我們也為 Linux、macOS 與 Windows 發行專用的自由執行緒 wheel：
