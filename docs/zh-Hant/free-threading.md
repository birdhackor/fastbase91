...
**先講結論：** 如果你為了真正的平行處理才換到自由執行緒 Python，只要匯入一個沒準備好的套件，CPython 就會把整個行程的 GIL 重新打開。CPython 會印出一則 `RuntimeWarning`，但匯入不會失敗，程式會繼續執行而執行緒重新被序列化；警告很容易被忽略，最後只會發現程式變慢。fastbase91 特別設計成：匯入它絕不會這樣坑你。
...
問題在這裡。編譯型擴充（帶有 C 或 Rust 部分的套件，例如 NumPy，或這個套件）必須「主動宣告」自己可以在沒有 GIL 的情況下執行。如果你匯入一個**沒有**宣告支援的擴充，CPython 為了安全，會把 **GIL 重新打開，而且是整個行程都打開**——不只是那個套件——並印出一則 `RuntimeWarning`；匯入不會失敗，程式會繼續執行。因此只要有一個還沒跟上的相依套件，就可能把你整支程式打回「一次只能跑一條執行緒」。

這則警告很容易被忽略，例如輸出被導走或淹沒在日誌裡。執行緒照跑，結果也照樣正確——只是又變回一條一條輪流跑，你當初想要的平行能力就沒了。`PYTHON_GIL=1` 或 `-X gil=1` 也可以重新開啟 GIL；用 `sys._is_gil_enabled()` 確認目前狀態。
