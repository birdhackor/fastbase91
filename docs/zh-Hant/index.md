...
- **可攜性。** 預先建好的 Python wheel 涵蓋 Linux（x86_64）、macOS（Apple silicon）與 Windows（x86_64）；在其他 CPython 平台上，pip 會嘗試從原始碼建置，需要 Rust 工具鏈（見[安裝與相容性](installation.md)）。Rust core 不需要 `std`、甚至不需要記憶體配置器（allocator）也能跑。
